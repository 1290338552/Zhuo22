...
if ($action=="insert"){
    ...
    $sql="select * from supplier where (equipmentNo='{$equipmentNo}' or supplierNo='{$supplierNo}')";
    $result=$db->db_select_count($sql);
    if ($result){       //添加，并判断是否有重复
        echo "<script> alert('商品类别编号重复或商品类别名称重复，请重新输入！');window.history.back();</script>";
        return;
    }
    if(strlen($supplierNo)>50){
        echo "<script> alert('商品类别编号不超过50个，类别名称不超过255个长度，请重新输入！');window.history.back();</script>";
        return;
    }

    $sql="INSERT INTO `supplier`(`equipmentNo`, `guige`, `unit`, `quantity`, `supplierNo`,`ArticleContent`,`BImgpath`)  VALUES ('{$equipmentNo}', '{$guige}', '{$unit}', '{$quantity}', '{$supplierNo}', '{$ArticleContent}', '{$BImgpath}')";
    $result=$db->db_insert_delete_update($sql);
    if ($result){
        echo "<script>alert('添加成功');  window.location.href='supplierList.php';</script>";
    }else{
    ...
    $sql="select * from supplier where (equipmentNo='{$equipmentNo}' or supplierNo='{$supplierNo}') and id<>'{$id}'";
...
//修改的类别编号、名称的长度超限，提示
    ...
    $sql="UPDATE `supplier` SET  `equipmentNo` = '{$equipmentNo}', `guige` = '{$guige}', `unit` = '{$unit}', `quantity` = '{$quantity}', `supplierNo` = '{$supplierNo}' , `ArticleContent` = '{$ArticleContent}' , `BImgpath` = '{$BImgpath}'  WHERE `id` = {$id}";
    $result=$db->db_insert_delete_update($sql);
    if ($result){
        echo "<script>alert('修改成功');  window.location.href='supplierList.php';</script>";
    }else{
    ...
    $result=$db->db_insert_delete_update($sql);
    if ($result){
        echo "<script language='JavaScript'>alert('删除成功');window.location.href='supplierList.php';</script>";
    }else{
    ...
    if (isset($_POST['DelBox'])&&!empty($_POST['DelBox'])){  //如果 有多选 并且 不空
        ...
        $sql="Delete from supplier where id in (".$id.")"; //Delete from Category where id in (2,3,4,5,6)   完成批量删除
        $result=$db->db_insert_delete_update($sql);
        if ($result){
            echo "<script language='JavaScript'>alert('批量删除成功');window.location.href='supplierList.php';</script>";
        }else{

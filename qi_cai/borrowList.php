...
<div class="wrapper">
                        ...
                        <div class="table-tools" style="margin-bottom: 15px;">
                            <div class="pull-right" style="width: 250px;">
                                <div class="input-group">
                                    <!--② 建个表单，和下面表单平行 -->        <form action="borrowList.php" name="SearchForm" method="post">
  ...
  <!--  ④ 单击事件，让表单提交 -->                 <button class="btn btn-default" onclick="document.SearchForm.submit()" type="button">搜索</button>
                            ...
                            <div class="tools-group">
                                <a href="borrowInsert.php" class="btn btn-primary"><i class="icon icon-plus-sign"></i> 新增</a>
                        ...
                        <!-- ① 从56行移下来的-->   <form action="borrowSave.php?action=DelAll" method="post" name="DelForm" >
                            <table class="table table-bordered">
                                ...
                                if($key){
                                    $sql="select  *  from  borrow where nameNo  like '%{$key}%' ";  //用like模糊查询
                                }else{
                                    $sql="select  *  from  borrow";  //无条件，读取所有记录
                                }
                                ...
                                foreach ($result as  $key=>$value){ //对每一行(二维数组的每行  记成  下标=>值)
                                    ...
                                    <tr>
                                        ...
                                        <td> <?php  echo  $value["borrowtime"]; ?> </td>
                                        <td> <?php  echo  $value["nameNo"]; ?> </td>
                                        <td>
                                            <a href="borrowUpdate.php?id=<?php  echo  $value["id"]; ?>" class="btn btn-xs btn-primary">编辑</a>
                                            <a href="borrowSave.php?action=delete&id=<?php  echo  $value["id"]; ?>" class="btn btn-xs btn-danger">删除</a>
                                        </td>
                                    </tr>
                                    <?php

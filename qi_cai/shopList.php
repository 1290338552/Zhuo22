...
<body>
 ...
 <!--② 建个表单，和下面表单平行 -->        <form action="shopList.php" name="SearchForm" method="post">
 <!--③ 给个名字 -->                      <input type="text" name="key"  class="form-control" placeholder="关键字" style="width: 180px">
  ...
  <!--  ④ 单击事件，让表单提交 -->                 <button class="btn btn-default" onclick="document.SearchForm.submit()" type="button">搜索</button>
                                ...
                                <div class="tools-group">
                                    <a href="shopInsert.php" class="btn btn-primary"><i class="icon icon-plus-sign"></i> 新增</a>
...
<!--                                           style="background-image: url('images/del2.png'); background-repeat: no-repeat;padding-left: 30px; ">-->
 ...
 <!-- ① 从56行移下来的-->   <form action="shopSave.php?action=DelAll" method="post" name="DelForm" >
 ...
 /*  ⑤   */                       $key=isset($_POST["key"]) ? $_POST["key"] : NULL ; //搜索框如果传来值。就接收，否则就是空
                                    if($key){
                                        $sql="select  *  from  shop where shopName  like '%{$key}%' ";  //用like模糊查询
                                    }else{
                                        $sql="select  *  from  shop";  //无条件，读取所有记录
                                    }
                                ...
                                ?>
                                    <tr>
                                        ...
                                        <td> <?php  echo  $value["shopNo"]; ?> </td>
                                        <td> <?php  echo  $value["shopName"]; ?> </td>
                                        <td> <?php  echo  $value["categoryNo"]; ?> </td>
                                        <td> <?php  echo  $value["shopMoney"]; ?> </td>
                                        <td> <img src="uploadfile/<?php  echo  $value["BImgpath"]; ?>" width="60" height="80"/>  </td>
                                        <td> <?php  echo  $value["shopDescp"]; ?> </td>
                                        <td> <?php  echo  $value["remark"]; ?> </td>
                                        <td>
                                            <a href="shopUpdate.php?id=<?php  echo  $value["id"]; ?>" class="btn btn-xs btn-primary">编辑</a>
                                            <a href="shopSave.php?action=delete&id=<?php  echo  $value["id"]; ?>" class="btn btn-xs btn-danger">删除</a>
                                        </td>
                                    </tr>
                                <?php

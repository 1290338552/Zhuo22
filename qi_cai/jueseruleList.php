...
<body>
  <form action="categorySave.php?action=DelAll" method="post" name="DelForm" >
    <div class="wrapper">
                        ...
                        <div class="panel-body">
                            <div class="table-tools" style="margin-bottom: 15px;">
                                ...
                                <div class="tools-group">
                                    <a href="categoryInsert.php" class="btn btn-primary"><i class="icon icon-plus-sign"></i> 新增</a>
                            ...
                            <table class="table table-bordered">
                                <thead>
                                    <tr>
                                        ...
                                        <th style="text-align: center">拥有权限的模块列表</th>
                                ...
                                //}
                                   ...
                                   $sql="select distinct jueseId  from  v_jueseRule order by jueseId";
                                   $result=$db->db_select_array($sql); //执行查询，读二维数组
                                   foreach ($result as  $key=>$item){ //对每一行(二维数组的每一行  记成  下标=>值)
                                       $sql2="select *  from  v_jueseRule where jueseId='{$item['jueseId']}' order by jueseId,ruleId";
                                       $result2=$db->db_select_array($sql2); //mysqli_num_rows($result2); 居然没有用
                                       $sql3="select count(jueseId) as rows  from  v_jueseRule where jueseId='{$item['jueseId']}' order by jueseId,ruleId";
                                   ...
                                   ?>
                                              <tr>
                                                  <td rowspan="<?=$rows?>" width="100" align="center"> <?php  echo  $value["jueseName"]; ?> </td>
                                                  <td width="300"> <?php  echo  $value["ruleName"]; ?> </td>
                                                  <td  width="60">
                                                      <a href="jueseRuleUpdate.php?jueseId=<?php  echo  $value["jueseId"];?>&ruleId=<?php  echo  $value["ruleId"];?>" class="btn btn-xs btn-primary">编辑</a>
                                                      <a href="jueseRuleSave.php?action=delete&jueseId=<?php  echo  $value["jueseId"];?>&ruleId=<?php  echo  $value["ruleId"];?>" class="btn btn-xs btn-danger">删除</a>
                                                  </td>
                                              </tr>
                                   <?php
                                          } else{

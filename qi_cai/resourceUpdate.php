...
$sqlS="select * from resource where id=$id "; //读取被修改的记录
$result=$db->db_select_oneRow($sqlS);
?>
<form action="upResourceSave.php?action=update&id=<?php  echo $result['id']; ?>" method="post" name="Form" enctype="multipart/form-data" >
    <div style="margin-left: auto;margin-right: auto;width: 400px; height: 650px; text-align: center;"> <!--页面居中-->
        ...
        <div style="width: 400px; height: auto;background-color: white;">
            <div style="width: 400px;height: 40px; border-bottom: 1px solid #7c7b7d;padding-top: 10px;margin-bottom: 20px;">
                <div style="width: 110px;height:38px;color: #333333;margin-left: auto;margin-right: auto;text-align: center; border-bottom: 3px solid #434343;">上传文件修改</div>
            </div>
            <div style="width: 400px;text-align: left;margin-left: 50px;">
                <input  type="text" name="resourceNo" value="<?php  echo $result['resourceNo']; ?>" placeholder="资源编号" required style="padding-left: 30px; background:url(images/person.jpg) no-repeat left  center;height: 40px;width: 260px;"><br/><br/>
                <input  type="text" name="resourceName" value="<?php  echo $result['resourceName']; ?>" id="passWord" placeholder="资源名称" required style="padding-left:30px;background:url(images/lock.jpg) no-repeat left  center;height: 40px;width: 260px; "><br/><br/>
                ...
                <select name="categoryNo" required style="padding-left: 30px; height: 40px;width: 260px;">
                    ...
                    foreach ($category as $key=>$item){  //对$result的每一行  当成  下标=>内容   内容就是一行记录
                        ?>
                        <option value="<?php echo $item['categoryNo'] ?>" <?php if($result['categoryNo']==$item['categoryNo']) echo 'selected'; ?> > <?php echo $item['categoryName']?> </option>
                        <?php
                    }
                    ?>
                </select> <br/><br/>

                <textarea name="introduction" id="introduction" style="width:99%;height:350px;visibility:hidden;"> <?php echo $result['introduction']?> </textarea> <br/>


                请修改上传文件：<input name="upFilePath" value="<?php echo $result['upFilePath']?>" type="file" size="30" />
                <input  type="text" name="remark" value="<?php  echo $result['remark']; ?>" id="passWord" placeholder="备注" required style="padding-left:30px;background:url(images/lock.jpg) no-repeat left  center;height: 40px;width: 260px; "><br/><br/>

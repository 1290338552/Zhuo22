...
if ($action=="insert"){
    ...
    $nameNo=$_POST["nameNo"];
    $sql="select * from name where (nameName='{$nameName}' or nameNo='{$nameNo}')";
    $result=$db->db_select_count($sql);
    if ($result){       //添加，并判断是否有重复
        echo "<script> alert('商品类别编号重复或商品类别名称重复，请重新输入！');window.history.back();</script>";
        return;
    }
    if(strlen($nameNo)>50 || strlen($nameName)>255){
        echo "<script> alert('商品类别编号不超过50个，类别名称不超过255个长度，请重新输入！');window.history.back();</script>";
        return;
    }

    $sql="INSERT INTO `name`(`nameName`, `nameNo`)  VALUES ('{$nameName}', '{$nameNo}')";
    $result=$db->db_insert_delete_update($sql);
    if ($result){
        echo "<script>alert('添加成功');  window.location.href='borrowuserList.php';</script>";
    }else{
        echo "<script>alert('添加失败');  window.history.back();</script>";
    }
}else if ($action=="update"){       //修改
    $id=$_GET["id"];        //地址栏传来，用get
    $nameName=$_POST["nameName"]; //表单里传来，用post
    $nameNo=$_POST["nameNo"];

    $sql="select * from name where (nameName='{$nameName}' or nameNo='{$nameNo}') and id<>'{$id}'";
...
//修改的类别编号、名称的长度超限，提示
    ...
    $sql="UPDATE `name` SET `nameName` = '{$nameName}', `nameNo` = '{$nameNo}' WHERE `id` = {$id}";
    $result=$db->db_insert_delete_update($sql);
    if ($result){
        echo "<script>alert('修改成功');  window.location.href='borrowuserList.php';</script>";
    }else{
    ...
    $result=$db->db_insert_delete_update($sql);
    if ($result){
        echo "<script language='JavaScript'>alert('删除成功');window.location.href='borrowuserList.php';</script>";
    }else{
    ...
    if (isset($_POST['DelBox'])&&!empty($_POST['DelBox'])){  //如果 有多选 并且 不空
        ...
        $sql="Delete from name where id in (".$id.")"; //Delete from name where id in (2,3,4,5,6)   完成批量删除
        $result=$db->db_insert_delete_update($sql);
        if ($result){
            echo "<script language='JavaScript'>alert('批量删除成功');window.location.href='borrowuserList.php';</script>";
        }else{

...
$sqlS="select * from storage where id=$id "; //读取被修改的记录
$result=$db->db_select_oneRow($sqlS);
?>
<form action="storageSave.php?action=update&id=<?php  echo $result['id']; ?>" method="post" name="Form" enctype="multipart/form-data" >
    <div style="margin-left: auto;margin-right: auto;width: 400px; height: 650px; text-align: center;"> <!--页面居中-->
        ...
        <div style="width: 400px; height: auto;background-color: white;">
            ...
            <div style="width: 400px;text-align: left;margin-left: 50px;">
                <input  type="text" name="storageNo" value="<?php  echo $result['storageNo']; ?>" placeholder="存储信息编号" required style="padding-left: 30px; background:url(images/person.jpg) no-repeat left  center;height: 40px;width: 260px;"><br/><br/>
                <input  type="text" name="storageName" value="<?php  echo $result['storageName']; ?>" id="passWord" placeholder="存储名称" required style="padding-left:30px;background:url(images/lock.jpg) no-repeat left  center;height: 40px;width: 260px; "><br/><br/>

                <select name="categoryNo" required style="padding-left: 30px; height: 40px;width: 260px;">
                    <option value="">--- 请选择存储类别 ---</option>
                    <?php

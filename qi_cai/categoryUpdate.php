...
<head>
    ...
    $sql="select  *  from  category  where  id={$id} ";//做条查询语句，把此id记录读出来
    $result=$db->db_select_oneRow($sql); //调用读一行
    //切记，下面的字段名都要改为自己表中对应的字段名
    ?>
</head>
<body style="background-color: #f3f3f3">
<form action="categorySave.php?action=update&id=<?php  echo $result['id']; ?>" method="post" name="Form" enctype="multipart/form-data" >
    <div style="margin-left: auto;margin-right: auto;width: 400px; height: 650px; text-align: center;"> <!--页面居中-->
      ...
      <div style="width: 400px; height: auto;background-color: white;">
          ...
          <div style="width: 400px;text-align: left;margin-left: 50px;">
              <input  type="text" name="categoryNo" value="<?php  echo $result['categoryNo']; ?>" placeholder="类别编号" required style="padding-left: 30px; background:url(images/person.jpg) no-repeat left  center;height: 40px;width: 260px;"><br/><br/>
              <input  type="text" name="categoryName" value="<?php  echo $result['categoryName']; ?>" id="passWord" placeholder="类别名称" required style="padding-left:30px;background:url(images/lock.jpg) no-repeat left  center;height: 40px;width: 260px; "><br/><br/>
          </div>
